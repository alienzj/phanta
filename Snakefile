...
rule merge_counts_normed: # make the 3 tables we want! :)
  input:
    to_merge_list = expand(join(outdir, "classification/{samp}.krak.report_bracken_S1.filtered.to_merge"), samp=sample_names),
    to_merge_norm_list = expand(join(outdir, "classification/{samp}.krak.report_bracken_S1.filtered.to_merge.norm_brack"), samp=sample_names)
  output:
